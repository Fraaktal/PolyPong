...
var balle = new Balle(canvas.width / 2, canvas.height / 2, canvas.width / 800, canvas.width / 200, Math.PI, 8);
...
connection.on("StartGame", function (idPlayer1, idPlayer2) {
    player1Id = idPlayer1;
	player2Id = idPlayer2;
...
connection.on("PauseAsked", function () {
    if (pause) {
        draw();
    }
    pause = !pause;
...
connection.on("ConnectAndDisplayWaitingConnexionScreen", function (idGame, uniqueIdToDisplay, isP2) {
	gameId = idGame;
	isPlayerP2 = isP2;
...
function playGame() {
    setInterval(function() {
            ...
            if (scoreJ1 === 5 && !fin) {
                ...
                context.fillRect(0, 0, canvas.width, canvas.height);
                context.font = "50px Courrier";
                context.fillStyle = "white";
                context.textAlign = "center";
                context.fillText("Victoire du joueur 1 !", canvas.width / 2, canvas.height / 2 - 20);
                context.fillText("Score : " + scoreJ1 + " a " + scoreJ2, canvas.width / 2, canvas.height / 2 + 20);
                fin = true;
                pause = true;
            } else if (scoreJ2 === 5 && !fin) {
                context.fillStyle = "rgba(0,0,0,1)";
                context.fillRect(0, 0, canvas.width, canvas.height);
                context.font = "50px Courrier";
                context.fillStyle = "white";
                context.textAlign = "center";
                context.fillText("Victoire du joueur 2 !", canvas.width / 2, canvas.height / 2 - 20);
                context.fillText("Score : " + scoreJ2 + " a " + scoreJ1, canvas.width / 2, canvas.height / 2 + 20);
                fin = true;
                pause = true;
            }
        },
...
function actionKeyCode(event) {
	...
	if (touche === 90 && !pause && !sombre) {
		...
		connection.invoke("Player_Up",gameId,player1Id).catch(function (err) {
            return console.error(err.toString());
        });
    }
	...
	if (touche === 83 && !pause && !sombre) {
		...
		connection.invoke("Player_Down", gameId, player1Id).catch(function (err) {
            return console.error(err.toString());
        });
	}
	...
	if (touche === 38 && !pause && !sombre) {
		...
		connection.invoke("Player_Up", gameId, player2Id).catch(function (err) {
            return console.error(err.toString());
        });
	}
	...
	if (touche === 40 && !pause && !sombre) {
		...
		connection.invoke("Player_Down", gameId, player2Id).catch(function (err) {
            return console.error(err.toString());
        });
	}
	//echap, mise en pause
	if (touche === 27) {
        connection.invoke("AskForPause", gameId).catch(function (err) {
            return console.error(err.toString());
        });
    }
}
...
function actionKeyCodeUp(event) {
	...
	if (touche === 90 && !pause && !sombre) {
		...
		connection.invoke("Player_StopMoving_Up", gameId, player1Id).catch(function (err) {
            return console.error(err.toString());
        });
	}
	if (touche === 83 && !pause && !sombre) {
		...
		connection.invoke("Player_StopMoving_Down", gameId, player1Id).catch(function (err) {
            return console.error(err.toString());
        });
    }
	if (touche === 38 && !pause && !sombre) {
		...
		connection.invoke("Player_StopMoving_Up", gameId, player2Id).catch(function (err) {
            return console.error(err.toString());
        });
	}
	if (touche === 40 && !pause && !sombre) {
		...
		connection.invoke("Player_StopMoving_Down", gameId, player2Id).catch(function (err) {
            return console.error(err.toString());
        });
	}
}
...
function detecRebond() {
	...
	else if (rebondBord && balle.positionY >= canvas.height - balle.taille - canvas.height / 10 && balle.angle < Math.PI) {
		rebondBord = false;
		return false;
	}
	if (!rebondRaquette && balle.positionX <= balle.taille + canvas.width / 20 && balle.positionX >= canvas.width / 20) {
		if (raquette1.position <= balle.positionY && raquette1.position + canvas.height / 5 >= balle.positionY) {
			...
			rebondRaquette = true;
            balle.angle = balle.angle - (raquette1.position + canvas.height / 10 - balle.positionY) * Math.PI / 2 * canvas.height / (100000 * canvas.height / 480);
        ...
        balle.miseEnJeu(canvas.width / 2, canvas.height / 2, canvas.width / 800, Math.PI);
		scoreJ2 += 1;
		return true;
	}
	else if (rebondRaquette && balle.positionX >= balle.taille + canvas.width / 20 + canvas.width / 10 && (balle.angle > 3 * Math.PI / 2 || balle.angle < Math.PI / 2)) {
	...
	if (!rebondRaquette && balle.positionX >= canvas.width - balle.taille - canvas.width / 20 && balle.positionX <= canvas.width - canvas.width / 20) {
		if (raquette2.position <= balle.positionY && raquette2.position + canvas.height / 5 >= balle.positionY) {
			balle.calculRebond(false);
			rebondRaquette = true;
            balle.angle = balle.angle + (raquette2.position + canvas.height / 10 - balle.positionY) * Math.PI / 2 * canvas.height / (100000 * canvas.height/480);
        ...
        balle.miseEnJeu(canvas.width / 2, canvas.height / 2, canvas.width / 800, 0);
		scoreJ1 += 1;
		return true;
	}
